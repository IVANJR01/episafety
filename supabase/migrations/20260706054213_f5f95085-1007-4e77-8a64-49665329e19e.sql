...
BEGIN
  ...
  FOR r IN
    ...
    especificos AS (
      SELECT
        ...
        ARRAY[fn.nome_funcao]::text[] AS funcoes_snapshot
      FROM base b
  ...
  LOOP
    ...
    IF NOT _dry_run THEN
      -- funcionarios não possui funcao_id; a ligação com função é feita por cargo = ghe_funcoes.nome_funcao
      IF r.funcao_id IS NOT NULL THEN
        SELECT nome_funcao INTO v_funcao_nome FROM public.ghe_funcoes WHERE id = r.funcao_id;
        SELECT count(*)::int INTO v_count FROM public.funcionarios f
          WHERE f.ghe_id = r.ghe_id
            AND lower(trim(coalesce(f.cargo,''))) = lower(trim(coalesce(v_funcao_nome,'')))
