...
-- Table: ghe_setores (setores por GES, com processo do setor)
CREATE TABLE IF NOT EXISTS public.ghe_setores (
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.ghe_setores TO authenticated;
GRANT ALL ON public.ghe_setores TO service_role;

ALTER TABLE public.ghe_setores ENABLE ROW LEVEL SECURITY;

CREATE POLICY ghe_setores_select ON public.ghe_setores FOR SELECT
  USING (is_super_admin(auth.uid()) OR is_principal(auth.uid()) OR is_in_user_company_tree(auth.uid(), empresa_id));
CREATE POLICY ghe_setores_insert ON public.ghe_setores FOR INSERT
  WITH CHECK (is_super_admin(auth.uid()) OR is_principal(auth.uid()) OR is_in_user_company_tree(auth.uid(), empresa_id));
CREATE POLICY ghe_setores_update ON public.ghe_setores FOR UPDATE
  USING (is_super_admin(auth.uid()) OR is_principal(auth.uid()) OR is_in_user_company_tree(auth.uid(), empresa_id));
CREATE POLICY ghe_setores_delete ON public.ghe_setores FOR DELETE
  USING (is_super_admin(auth.uid()) OR is_principal(auth.uid()) OR is_in_user_company_tree(auth.uid(), empresa_id));

CREATE INDEX IF NOT EXISTS idx_ghe_setores_ghe ON public.ghe_setores(ghe_id);
CREATE INDEX IF NOT EXISTS idx_ghe_setores_empresa ON public.ghe_setores(empresa_id);

CREATE TRIGGER trg_ghe_setores_updated
  BEFORE UPDATE ON public.ghe_setores
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- Migração dos setores antigos (array em ghe_ges.setores) para a nova tabela, sem processo.
INSERT INTO public.ghe_setores (empresa_id, ghe_id, nome, processo)
SELECT g.empresa_id, g.id, s AS nome, NULLIF(trim(g.processo),'')
  FROM public.ghe_ges g
       CROSS JOIN LATERAL unnest(COALESCE(g.setores, ARRAY[]::text[])) AS s
 WHERE s IS NOT NULL AND trim(s) <> ''
ON CONFLICT (ghe_id, nome) DO NOTHING;

-- Atualiza pgr_importar_ghe: fallback de processo agora é Função → Setor(ghe_setores) → GES.
