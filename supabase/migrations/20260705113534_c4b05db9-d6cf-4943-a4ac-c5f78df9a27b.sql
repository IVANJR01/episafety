...
-- 1) Nova coluna funcao_id no inventário
ALTER TABLE public.pgr_inventario_itens
  ADD COLUMN IF NOT EXISTS funcao_id uuid REFERENCES public.ghe_funcoes(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_pgr_inv_funcao_id ON public.pgr_inventario_itens(funcao_id);

-- 2) RPC atualizada: expande por função
...
BEGIN
  ...
  LOOP
      ...
      ELSE
        ...
        IF NOT _dry_run THEN
          ...
          INSERT INTO public.pgr_inventario_itens(
            ...
            v_aval, v_sev, v_prob, v_ctrls, auth.uid()
          );
        END IF;
      END IF;
    ...
    ELSIF v_has_funcoes THEN
      ...
      FOR f IN SELECT id FROM public.ghe_funcoes WHERE ghe_id = r.ghe_id LOOP
        SELECT EXISTS (
          SELECT 1 FROM public.pgr_inventario_itens i
           WHERE i.pgr_id = _pgr_id
             AND i.ghe_id = r.ghe_id
    ...
    ELSE
      -- Caso 3: GES sem funções — comportamento antigo (item único sem funcao_id)
      SELECT EXISTS (
        SELECT 1 FROM public.pgr_inventario_itens i
         WHERE i.pgr_id = _pgr_id
           AND i.ghe_id = r.ghe_id
