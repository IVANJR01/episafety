...
-- ============================================================================
-- CONFIRMADO NO BANCO (consulta direta em 30/07)
--
-- A tabela public.conformidades não tem NENHUM registro das duas empresas CG3.
-- Nem Barrocas, nem Jardim, nem nada. Os registros existentes pertencem a
-- outras empresas (G91 e outra), com números de 3 a 51. Ou seja: as inspeções
-- da CG3 nunca chegaram ao banco — não foram apagadas agora. O que a tela
-- mostrava vinha da cópia guardada no navegador e dos registros escritos
-- dentro do código.
--
-- Confirmado também que nenhum registro da tabela, de nenhuma empresa, tem
-- foto gravada (foto_antes_path está vazio em todos).
--
-- ORIGEM DESTES DADOS
--
-- Relatório de Inspeção de Segurança emitido em 07/07/2026 às 09:35, período
-- de 01/06/2026 a 07/07/2026, 22 inspeções. Cada campo foi lido do próprio
-- PDF. Os rodapés de página que o extrator grudou no fim de seis textos
-- (registros 3, 6, 9, 12, 15 e 20) foram removidos.
--
-- POR LOCAL
--    9  ALOJAMENTO - ALTO RODRIGUES
--    6  SE 69 KV BARROCAS     <-- números 1, 2, 3, 4, 5 e 22
--    5  ALOJAMENTO - MOSSORO
--    2  SE 69 KV ESTREITO
--
-- Os números 1 a 22 estão livres: só o id é único nesta tabela, e os números
-- em uso pelas outras empresas vão de 3 a 51 — sem conflito de identidade.
--
-- EMPRESA DE DESTINO
--
...
-- AS FOTOS
--
-- As 22 fotos "ANTES" foram recuperadas do PDF e entregues em arquivo à parte
-- (insp_001.jpg … insp_022.jpg). Este script grava só os dados; as fotos
-- precisam ser anexadas pela tela.
--
...
BEGIN
  ...
  VALUES
    ...
    ('c3300000-0000-4000-8000-000000000001', 1, 'PENDENTE', 'MODERADO', '2026-07-03'::date,
     ...
     'NR-18 — Item 18.6.10 — Quadros de distribuição devem possuir partes vivas inacessíveis e protegidas contra acesso de trabalhadores não autorizados.', v_empresa, '2026-07-03'::timestamptz),
    ('c3300000-0000-4000-8000-000000000002', 2, 'PENDENTE', 'MODERADO', '2026-07-03'::date,
     ...
     'NR-10 — Item 10.4.2.1 — Partes vivas das instalações elétricas devem ser protegidas contra contato acidental.', v_empresa, '2026-07-03'::timestamptz),
    ('c3300000-0000-4000-8000-000000000003', 3, 'PENDENTE', 'LEVE', '2026-07-03'::date,
     ...
     'Substituir ou reparar imediatamente as portas danificadas dos armários. Instalar ou restaurar os dispositivos para cadeado em todos os armários, garantindo a segurança dos pertences e a adequada guarda dos EPIs.',
     'NR-24 — Item 24.2.1.1 — Armários individuais devem possuir fechadura ou dispositivo para cadeado. NR-24 — Item 24.2.3.1 — Armários devem ser mantidos em bom estado de conservação.', v_empresa, '2026-07-03'::timestamptz),
    ('c3300000-0000-4000-8000-000000000004', 4, 'PENDENTE', 'MODERADO', '2026-07-03'::date,
     ...
     'NR-10 — Item 10.4.1(a) — As instalações elétricas devem possuir diagramas unifilares atualizados.', v_empresa, '2026-07-03'::timestamptz),
    ('c3300000-0000-4000-8000-000000000005', 5, 'PENDENTE', 'RISCO CRÍTICO', '2026-07-03'::date,
     ...
     'NR-06 — Item 6.3(III) — Empregador deve substituir imediatamente EPI danificado.', v_empresa, '2026-07-03'::timestamptz),
    ('c3300000-0000-4000-8000-000000000006', 6, 'PENDENTE', 'MODERADO', '2026-07-06'::date,
     ...
     'Realizar organização imediata do almoxarifado, implementando um programa de 5S. Demarcar áreas de armazenamento e circulação. Treinar os colaboradores sobre os procedimentos de organização e manutenção do local.',
     'NR-11 — Item 11.2.1 — Armazenamento de materiais deve obedecer a requisitos de segurança. NR-11 — Item 11.2.4 — Materiais não podem impedir a circulação de pessoas.', v_empresa, '2026-07-06'::timestamptz),
    ('c3300000-0000-4000-8000-000000000007', 7, 'PENDENTE', 'MODERADO', '2026-07-06'::date,
     ...
     'NR-18 — Item 18.6.13 — Proteção de circuitos elétricos contra contatos acidentais e danos mecânicos. NR-10 — Item 10.2.8.2 — Proteção de cabos e condutores contra impactos mecânicos e intempéries.', v_empresa, '2026-07-06'::timestamptz),
    ('c3300000-0000-4000-8000-000000000008', 8, 'PENDENTE', 'LEVE', '2026-06-08'::date,
     ...
     'Checklist alojamento NEOENERGIA', v_empresa, '2026-06-08'::timestamptz),
    ('c3300000-0000-4000-8000-000000000009', 9, 'PENDENTE', 'RISCO CRÍTICO', '2026-06-08'::date,
     ...
     'Realizar a instalação imediata de sistema de iluminação de emergência em todas as rotas de fuga e áreas de permanência, seguindo as diretrizes da NBR 10898.',
     'NR-23 — Item 23.11.1 — Instalação de iluminação de emergência em locais de trabalho.', v_empresa, '2026-06-08'::timestamptz),
    ('c3300000-0000-4000-8000-000000000010', 10, 'PENDENTE', 'MODERADO', '2026-06-08'::date,
     ...
     'NR-10 — Item 10.4.1 — Instalações elétricas devem ser construídas e montadas de forma segura para prevenir riscos.', v_empresa, '2026-06-08'::timestamptz),
    ('c3300000-0000-4000-8000-000000000011', 11, 'PENDENTE', 'RISCO CRÍTICO', '2026-06-08'::date,
     ...
     'NR-24 — Item 24.1.1 — Manutenção das condições sanitárias e de conforto nos locais de trabalho.', v_empresa, '2026-06-08'::timestamptz),
    ('c3300000-0000-4000-8000-000000000012', 12, 'PENDENTE', 'MODERADO', '2026-06-08'::date,
     ...
     'Realizar a instalação imediata de canaletas, conduítes ou outro tipo de proteção mecânica adequada para todos os fios expostos. Garantir que não existam partes vivas acessíveis e que os condutores estejam organizados para eliminar riscos de choque elétrico e tropeços, especialmente na área da escada de saída.',
     'NR-18 — Item 18.6.19 — Condutores elétricos devem ser protegidos e organizados para evitar acidentes. NR-18 — Item 18.6.2 — Proibição de partes vivas expostas. NR-10 — Item 10.4.1 — Instalações elétricas devem prevenir acidentes.', v_empresa, '2026-06-08'::timestamptz),
    ('c3300000-0000-4000-8000-000000000013', 13, 'PENDENTE', 'MODERADO', '2026-06-08'::date,
     ...
     'NR-10 — Item 10.4.1 — Instalações elétricas devem ser construídas, montadas, operadas, inspecionadas e mantidas para prevenir acidentes. NR-10 — Item 10.4.2 — Instalações elétricas devem ser projetadas e construídas com segurança para trabalhadores e usuários.', v_empresa, '2026-06-08'::timestamptz),
    ...
    ('c3300000-0000-4000-8000-000000000016', 16, 'PENDENTE', 'GRAVE', '2026-06-08'::date,
     ...
     'NR-10 — Item a confirmar pelo responsável técnico — Adequação das instalações elétricas para garantir a segurança.', v_empresa, '2026-06-08'::timestamptz),
    ('c3300000-0000-4000-8000-000000000017', 17, 'PENDENTE', 'LEVE', '2026-07-06'::date,
     ...
     NULL, v_empresa, '2026-07-06'::timestamptz),
    ('c3300000-0000-4000-8000-000000000018', 18, 'PENDENTE', 'RISCO CRÍTICO', '2026-06-14'::date,
     ...
     'Instalar chuveiro',
     'NR-10 — Item 10.4.1 — Instalações elétricas devem prevenir acidentes elétricos e assegurar segurança e saúde. NR-10 — Item 10.4.2.1(c) — Medidas de proteção coletiva: Dispositivos Diferenciais Residuais (DR). NR-10 — Item 10.4.2.1(f) — Medidas de proteção coletiva: Aterramento.', v_empresa, '2026-06-14'::timestamptz),
    ('c3300000-0000-4000-8000-000000000019', 19, 'PENDENTE', 'LEVE', '2026-06-14'::date,
     'ALOJAMENTO - MOSSORO', 'OPERACIONAL',
     'Retirar ar-condicionado da garagem',
     'Retirar ar-condicionado da garagem',
     'N.A', v_empresa, '2026-06-14'::timestamptz),
    ('c3300000-0000-4000-8000-000000000020', 20, 'PENDENTE', 'GRAVE', '2026-06-14'::date,
     'ALOJAMENTO - MOSSORO', 'EQUIPE ELÉTRICA',
     'Tomada 220 volts danificada',
     'Desenergizar imediatamente o circuito, isolar a tomada danificada e providenciar sua substituição por eletricista qualificado. A tomada não deve ser utilizada antes da correção.',
     'NR-10 — Item 10.4.1 — Instalações elétricas seguras e íntegras.', v_empresa, '2026-06-14'::timestamptz),
    ('c3300000-0000-4000-8000-000000000021', 21, 'PENDENTE', 'GRAVE', '2026-06-14'::date,
     ...
     'NR-24 — Item 24.5.2(c) — Camas ou beliches devem possuir roupas de cama limpas.', v_empresa, '2026-06-14'::timestamptz),
    ('c3300000-0000-4000-8000-000000000022', 22, 'PENDENTE', 'MODERADO', '2026-06-03'::date,
     ...
     'NR-23 — Item 23.4.1 — Sistemas de alarme devem ser audíveis em todas as áreas e possuir código de identificação da emergência.', v_empresa, '2026-06-03'::timestamptz)
...
SELECT local, count(*) AS registros, min(numero) AS de, max(numero) AS ate
 ...
 WHERE empresa_id = '75447c33-0960-46db-ba59-00327575fe44'
 GROUP BY local
 ORDER BY local;
