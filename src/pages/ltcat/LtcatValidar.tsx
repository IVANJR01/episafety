...
import { LTCAT_STATUS_LABEL, LTCAT_STATUS_COLOR, LtcatStatus } from "@/lib/ltcatTypes";

export default function LtcatValidar() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["ltcat-validar", id, versaoQs],
    queryFn: async () => {
      const { data, error } = await (supabase.rpc as any)("ltcat_validar_interno", {
        _ltcat_id: id, _pdf_versao: versaoQs ?? null,
      });
  ...
  useEffect(() => {
    if (!isLoading && !data && error) {
      toast.error("LTCAT não encontrado ou fora do seu escopo.");
      navigate("/ltcat");
    }
  ...
  const status = data.status as LtcatStatus;
  const pdf = data.pdf;
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground ml-auto">Acesso restrito à empresa do LTCAT</span>
      </div>

      <Card>
        <CardHeader>
          <div className="flex items-center justify-between flex-wrap gap-2">
            <CardTitle className="text-xl">LTCAT v{data.versao}</CardTitle>
        ...
        <CardContent className="space-y-3 text-sm">
          {desatualizado && (
            <div className="rounded-md p-3 text-sm flex items-center gap-2 bg-amber-50 border border-amber-200 text-amber-800">
              ...
              PDF desatualizado — o LTCAT foi alterado após esta versão. Regenere o PDF.
            </div>
          )}
          {versaoDivergente && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            <Info label="CNPJ" value={data.empresa_cnpj} />
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <Info label="Status no momento" value={pdf ? LTCAT_STATUS_LABEL[pdf.status_no_momento as LtcatStatus] : "—"} />
          <div>
      ...
      <div className="flex justify-between flex-wrap gap-2">
        <Button asChild variant="outline">
          <Link to={`/ltcat/${id}`}>Ir para o LTCAT completo <ArrowRight className="h-4 w-4 ml-1" /></Link>
        </Button>
      </div>
    </div>
  );
